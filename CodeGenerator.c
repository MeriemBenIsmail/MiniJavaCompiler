...
int addCodeVariable(char *code_op, char *name)
{
    ...
    for (int i = 0; i <= getLastCode(); i++)
    {
        if (tab_code_int[i].operande == index)
        {
            ENTREE_CODE *entre = &tab_code_int[getLastCode()];
...
int addCodeLDV(char *code_op, char *name)
{
    ...
    for (int i = getLastCode(); i >= 0; i--)
    {
        if (tab_code_int[i].operande == index && strcmp(tab_code_int[i].designation, "") == 0)
        {
            addCodeLine("LDV", tab_code_int[i + 1].operande, "", "VALUE_FETCHING");
            break;
        }
    }
}

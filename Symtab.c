...
void setParam(char *name, char *type)
{
    ...
    NODE *function = &symbol_table[getLastElement()];
...
// method call param
void insertCallParam(int f_index, char *val, char *type)
{
...
int checkVariable(char *name, char *type, int level, int class_id)
{
    ...
    int x =addNode(name, "DECLARATION", "VARIABLE", type, level, class_id);
    return x;
}
...
int checkClass(char *name, int class_id)
{
    if (first_empty == 0)
    {
        return addNode(name, "DECLARATION", "CLASS", "NOTYPE", 0, class_id);
    }
    else
    {
        ...
        return addNode(name, "DECLARATION", "CLASS", "IDENT", 0, class_id);
    }
}
            ...
            if (strcmp(symbol_table[i].kind, "METHOD") == 0)
            {
                for (int j = 0; j < symbol_table[i].param_index; j++)
                {
                    if (strcmp(symbol_table[i].params_names[j], name) == 0)
                    {
                        ...
                        return addNode(name, function, kind, "NOTYPE", level, class_id);
                        test = 1;
                        break;
                    }
                }
    ...
    for (int i = 0; i <= getLastElement(); i++)
    {
        if (strcmp(symbol_table[i].kind, "METHOD") == 0 && strcmp(symbol_table[i].function, "USE") == 0)
        {
            ...
            if (strcmp(node->type, "NEW_IDENT") == 0)
            {
                int index = -1;
                int cid = -1;
                for (int j = 0; j <= getLastElement(); j++)
                {
                ...
                int test = 0;
                while (symbol_table[index].class_id == cid && index <= getLastElement())
                {
